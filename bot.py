...
def calculate_price(message):
    ...
    if skin_type == "Supreme":
        bot.send_message(message.chat.id, "✅ اسکین‌های Supreme هر کدام به ارزش تقریبی ۱,۲۰۰,۰۰۰ تومان محاسبه می‌شوند.\n❌ اسکین‌های رایگان در این بخش لحاظ نمی‌شوند.")
        bot.send_message(message.chat.id, "✅ لطفاً تعداد اسکین‌های Supreme خود را وارد کنید:")
        bot.register_next_step_handler(message, lambda msg: process_skin_count(msg, skin_type))

    elif skin_type == "Grand":
        bot.send_message(message.chat.id, "✅ اسکین‌های Grand هر کدام به ارزش تقریبی ۵۰۰,۰۰۰ تومان محاسبه می‌شوند.\n❌ اسکین‌های رایگان در این بخش لحاظ نمی‌شوند.")
        bot.send_message(message.chat.id, "✅ لطفاً تعداد اسکین‌های Grand خود را وارد کنید:")
        bot.register_next_step_handler(message, lambda msg: process_skin_count(msg, skin_type))

    elif skin_type == "Exquisite":
        bot.send_message(message.chat.id, "✅ اسکین‌های Exquisite هر کدام به ارزش تقریبی ۳۰۰,۰۰۰ تومان محاسبه می‌شوند.\n❌ اسکین‌های رایگان در این بخش لحاظ نمی‌شوند.")
        bot.send_message(message.chat.id, "✅ لطفاً تعداد اسکین‌های Exquisite خود را وارد کنید:")
        bot.register_next_step_handler(message, lambda msg: process_skin_count(msg, skin_type))

    elif skin_type == "Deluxe":
        bot.send_message(message.chat.id, "✅ اسکین‌های Deluxe به‌صورت زیر محاسبه می‌شوند:\n- کمتر از ۲۰ اسکین: هر اسکین ۲۵۰,۰۰۰ تومان\n- بین ۲۰ تا ۴۰ اسکین: ۵۰۰,۰۰۰ تومان ثابت\n- بالای ۴۰ اسکین: ۷۰۰,۰۰۰ تومان ثابت")
        bot.send_message(message.chat.id, "✅ لطفاً تعداد اسکین‌های Deluxe خود را وارد کنید:")
        bot.register_next_step_handler(message, lambda msg: process_skin_count(msg, skin_type))
